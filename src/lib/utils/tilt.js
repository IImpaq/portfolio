...
export default function tilt(node, overwriteSettings) {
    ...
    const doTransition = () => {
        ...
        node.style.transition = `${TRANSITION_DURATION}ms`;
        transitionId = setTimeout(() => node.style.transition = "0ms", TRANSITION_DURATION);
    }

    function onMouseMove(event) {
        const settings = { scale: 1, limit: 30, ...overwriteSettings };
